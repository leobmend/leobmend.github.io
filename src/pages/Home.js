...
function Home() {
  ...
  const MENU_LINKS_STYLE = 'ease-in duration-200 mx-20 my-4 rounded-xl p-2 font-semibold '
  ...
  return (
    <main className="flex flex-col justify-center items-center h-screen w-screen">
      <div
        className={ 'ease-in-out duration-700 h-screen w-screen items-center justify-center flex '
          + `flex-col ${upContainerStyle} md:flex-row` }
      >
        <div className="flex flex-col my-4 mx-4 md:mx-8 2xl:mx-20">
          <h1 className="text-center font-bold text-white text-4xl md:text-6xl 2xl:text-7xl">
            LEONARDO
          </h1>
          <h1 className="text-center text-white text-[1.4rem] md:text-[2.1rem] 2xl:text-[2.5rem]">
            BARROS MENDONÇA
          </h1>
        </div>
        <img
          className={ 'ease-in-out duration-700 rounded-full my-4 mx-4 w-5/12 md:w-4/12 xl:w-3/12 '
           + '2xl:w-2/12 md:mx-8 2xl:mx-20 ' }
          src={ profilePic }
          alt="Leonardo"
        />
      </div>
      ...
      />
      <nav className={ 'ease-in-out duration-700 flex flex-col justify-center items-center '
        + `text-white h-full z-10 text-3xl ${downContainerStyle} 2xl:text-4xl 2xl:flex-row` }
      >
