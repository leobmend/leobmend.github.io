  ...
  return (
    <main className="flex flex-col justify-center items-center overflow-hidden h-full">
      <MenuNav
        ...
        actualPage="Technologies"
      />
      <div
        className={ 'ease-in-out duration-700 h-full w-full flex items-center justify-center '
          + `${upContainerStyle}` }
      >
        <div
          className={ 'flex items-center overflow-x-scroll h-5/6 snap-x snap-mandatory '
            + '2xl:scrollbar2xl' }
        >
          <TechLogo
            logo={ jsLogo }
            selected={ techSelected === 'js' }
            handleClick={ () => handleClickTech('js') }
            styles="w-5/6"
          />
          <TechLogo
            logo={ cssLogo }
            selected={ techSelected === 'css' }
            handleClick={ () => handleClickTech('css') }
          />
          <TechLogo
            logo={ htmlLogo }
            selected={ techSelected === 'html' }
            handleClick={ () => handleClickTech('html') }
          />
          <TechLogo
            logo={ nodeLogo }
            selected={ techSelected === 'node' }
            handleClick={ () => handleClickTech('node') }
            styles="w-5/6"
          />
          <TechLogo
            logo={ jestLogo }
            selected={ techSelected === 'jest' }
            handleClick={ () => handleClickTech('jest') }
            styles="w-5/6"
          />
          <TechLogo
            logo={ mochaLogo }
          ...
          <TechLogo
            logo={ reactLogo }
            selected={ techSelected === 'react' }
            handleClick={ () => handleClickTech('react') }
          />
          <TechLogo
            logo={ reduxLogo }
            selected={ techSelected === 'redux' }
      ...
      />
      <div
        className={ `ease-in-out duration-500 h-full w-full flex items-center justify-center '
          + ${downContainerStyle}` }
      >
